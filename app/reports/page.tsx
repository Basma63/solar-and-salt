...
import { supabase, DailyDiesel, DailySalt, DieselTransaction } from '@/lib/supabase';
...
export default function ReportsPage() {
  ...
  const fetchReports = async () => {
    ...
    try {
      // Fetch diesel data — continuous inventory from transactions
      ...
      // All transactions up to end of range, ordered by date
      const { data: allDieselTx } = await supabase
      ...
      const dieselTxList = (allDieselTx || []) as DieselTransaction[];

      // Opening = initial + sum(all tx strictly before start)
      let opening = initBal;
      for (const tx of dieselTxList) {
        if (tx.date < start) {
          opening += tx.type === 'received' ? Number(tx.quantity) : -Number(tx.quantity);
        }
      }

      // Received / distributed within [start, end]
      const inRange = dieselTxList.filter(t => t.date >= start && t.date <= end);
      const totalReceived = inRange
        .filter(t => t.type === 'received')
        .reduce((s, t) => s + Number(t.quantity), 0);
      const totalDistributed = inRange
        .filter(t => t.type === 'distributed')
        .reduce((s, t) => s + Number(t.quantity), 0);
      const remaining = opening + totalReceived - totalDistributed;
